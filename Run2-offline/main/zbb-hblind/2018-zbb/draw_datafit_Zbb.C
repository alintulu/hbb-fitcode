...
void draw(int index, bool pass, bool is_ggf, bool log=true){
  ...
  Zjetsbb->Scale(7.0);
  //Zjetsbb->Scale(rZbb);
  Zjetsbb->SetLineColor(kRed+1);
  Zjetsbb->SetMarkerColor(kRed+1);
  Zjetsbb->SetLineStyle(2);
  Zjetsbb->SetLineWidth(3);

  /* bkg Higgs */
  TH1D* bkgHiggs = (TH1D*)Zjetsbb->Clone("bkgHiggs");
  bkgHiggs->Reset();
  ...
  /* single t */
  TH1D* singlet = (TH1D*)Zjetsbb->Clone("singlet");
  singlet->Reset();
  singlet->Add((TH1D*)f->Get((histdirname+"singlet").c_str()));
  singlet->Scale(7.0);
  singlet->SetLineWidth(1);
  ...
  double max = data_obs->GetMaximum();
  double xmax = 201;
  double xmin = 40;
  ...
  TH1D* Zjetsbb_sub = (TH1D*)Zjetsbb->Clone("Zjetsbb_sub");
  Zjetsbb_sub->Reset();
  ...
  for(int i=1; i<TotalBkg_sub->GetNbinsX()+1; i++){
    ...
    Zjetsbb_sub->SetBinContent(i,Zjetsbb->GetBinContent(i)/data_obs->GetBinError(i));
  }
